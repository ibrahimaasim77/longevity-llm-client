...
API_KEY = os.environ.get("HF_TOKEN") or os.environ.get("LONGEVITY_API_KEY")
if not API_KEY:
    print("No HF_TOKEN env var found.")
    print("Get the hackathon token from the organizers, then either:")
    print("  export HF_TOKEN=hf_xxx     # one-time per shell")
    print("  or paste it now (not echoed).")
    API_KEY = getpass("HF token: ").strip()
    if not API_KEY:
